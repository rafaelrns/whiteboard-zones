...
export function FabricBoard({ className, __onCanvas, __onZoneRect, __zones, __onCanvasJson, __applyRemoteJson, __onPointer, __lockedObjectIds, __isNewBoard }) {
    ...
    useEffect(() => {
        ...
        const ensureOid = (o) => {
            if (!o.__oid)
                o.__oid = `obj-${Date.now()}-${Math.random().toString(36).slice(2)}`;
        };
        ...
        const updateLinkedArrows = (movedObj) => {
            const oid = movedObj.__oid;
            if (!oid)
                return;
        ...
        const onMouseDown = (opt) => {
            ...
            if (toolRef.current === 'rect') {
                ...
                if (origin.type === 'object' && target && !target.__zone) {
                    arrow.__fromId = origin.obj.__oid;
                    arrow.__toId = target.__oid;
                }
                ensureOid(arrow);
                c.add(arrow);
            ...
            if (mod && e.key.toLowerCase() === 'g') {
                ...
                if (e.shiftKey) {
                    if (active instanceof fabric.Group || active.type === 'group') {
                        const g = active;
                        if (typeof g.ungroupOnCanvas === 'function') {
                            g.ungroupOnCanvas();
                        }
                        else {
