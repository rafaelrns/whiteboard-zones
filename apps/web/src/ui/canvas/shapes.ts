...
export function createRoundedRect(left: number, top: number, width: number, height: number) {
...
export function createDiamond(left: number, top: number, width: number, height: number) {
  const w2 = width / 2;
  const h2 = height / 2;
  const points = [
    { x: w2, y: 0 },
  ...
  (p as any).__shapeType = 'diamond';
  return p;
}

/** Paralelogramo - entrada/saída (pontos relativos a left,top) */
export function createParallelogram(left: number, top: number, width: number, height: number) {
  ...
  (p as any).__shapeType = 'parallelogram';
  return p;
}

/** Cilindro - banco de dados (pílula: retângulo com rx grande) */
export function createCylinder(left: number, top: number, width: number, height: number) {
  ...
  (r as any).__shapeType = 'cylinder';
  return r;
}

/** Nuvem - internet/rede (Path SVG com curvas Bézier, 3 lobos no topo, base plana) */
const CLOUD_PATH =
  'M 12,38 C 4,38 0,30 4,24 C 8,18 16,14 24,18 C 30,12 42,12 48,20 C 58,16 72,22 70,34 C 78,40 76,48 64,46 L 20,46 C 8,48 2,44 12,38 Z';

export function createCloud(left: number, top: number, width: number, height: number) {
  const w = Math.max(width, 40);
  const h = Math.max(height, 30);
  const path = new (fabric.Path as any)(CLOUD_PATH, {
    left,
    top,
    originX: 'left',
    originY: 'top',
